...
{
    public class SqlAsyncOrmDbHelper : IAsyncOrmDbHelper
    {
        private readonly IAsyncDbHelper _asyncDbHelper;

        public SqlAsyncOrmDbHelper(IAsyncDbHelper syncDbHelper) => _asyncDbHelper = syncDbHelper;

        public async Task<int> Insert(string tablename, IDictionary<string, object> parametres, DbTransaction tx)
        {
            ...
            return Convert.ToInt32(await _asyncDbHelper.ExecuteScalarAsync(query, tx, parametres));
        }

        public async Task Update(string tablename, IDictionary<string, object> parametres, DbTransaction tx, params Filter[] filters)
        {
            ...
            await _asyncDbHelper.ExecuteNonQueryAsync(query, tx, parametres);
        }

        public async Task<IList<T>> JoinedGet<T>(string tablename, string[] columns, Join[] joins, Func<IDataReader, T> entityReader, params Filter[] filters)
        {
            ...
            return await _asyncDbHelper.GetDataAsync(query, entityReader, parameters);
        }

        public async Task<IList<T>> SimpleGet<T>(string tablename, string[] columns, Func<IDataReader, T> entityReader, params Filter[] filters)
        {
